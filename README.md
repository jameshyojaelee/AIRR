...
- **Data layout**:
  ...
  - We ignore raw TSVs and data folders in git (see `.gitignore`).
- **Models we try**:
...
- **Pipelines**:
  ...
  - Submit scripts: `scripts/submit_hpc_runs.sh` (GPU) and `scripts/submit_hpc_runs_cpu.sh` (CPU) for Slurm arrays.
  - Outputs go to `outputs/<run>-<date>-<cpu|gpu>/`, each with per-dataset artifacts and a `run_summary.csv`.
- **Configs** (in `configs/`): `kmer_run1.json`, `gbm_run1.json`, `deepmil_run1.json` hold paths, model params, and submission settings.
- **Requirements**: numpy, pandas, scikit-learn, torch (for deep_mil), joblib, pyyaml; xgboost/lightgbm if installed for GBM speed.
